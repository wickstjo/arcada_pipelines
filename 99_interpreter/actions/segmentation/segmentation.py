...
def train_test_validation_split(segment_ratios: list[dict], dataset: list[dict], labels: list[float]):
    ...
    assert isinstance(labels, list), f"ARG 'dataset' MUST BE OF TYPE LIST[DICT]"

    assert len(dataset) == len(labels), f'DATASET AND LABELS ARE OF UNEQUAL LENGTH ({len(dataset)} != {len(labels)})'
    ...
    for block in segment_ratios:
        for segment_name, segment_percentage in block.items():
            ...
            container[segment_name] = {
                'features': dataset[old_limit:new_limit],
                'labels': labels[old_limit:new_limit],
            }
