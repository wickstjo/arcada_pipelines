...
class pipeline_component:
    ...
    def on_mlflow_change(self, latest_model_versions: dict):
        try:
            assert isinstance(latest_model_versions, dict), '[COMPONENT] REDIS VALUE WAS NOT A DICT'
            ...
            for stock_symbol, stock_pipes in self.deployed_model_pipes.items():
                for pipe_name, model_sequence in stock_pipes.items():
                    for nth_model, model in enumerate(model_sequence):
                        ...
                        assert model.model_name in latest_model_versions, f"[COMPONENT] MODEL '{model.model_name}' NO LONGER EXISTS"
                        ...
                        if model.version_alias:
                            ...
                            assert model.version_alias in latest_model_versions[model.model_name], f"[COMPONENT] VERSION ALIAS '{model.version_alias}' NO LONGER EXISTS FOR MODEL '{model.model_name}'"
                            ...
                            if old_version != newest_version:
                                ...
                                misc.log(f"[COMPONENT] UPDATED MODEL '{model.model_name}' VERSION ({old_version} -> {newest_version}) IN PIPE '{pipe_name}'")

                        # OTHERWISE, VERIFY THAT THE NUMERIC VERSION STILL EXISTS
                        # else:
                        #     assert self.mlflow.model_exists(model.model_name, model.version_number), f"[COMPONENT] MODEL '{model.model_name}' VERSION {model.version_number} NO LONGER EXISTS"

            misc.log('[COMPONENT] UPDATED PIPELINE STATE (MLFLOW)')

        except AssertionError as error:
            misc.log(error)
            misc.log('[COMPONENT] REVERTED PIPELINE STATE UPDATE (MLFLOW)')
    ...
    def on_redis_change(self, latest_model_pipelines: dict):
        try:
            assert isinstance(latest_model_pipelines, dict), '[COMPONENT] REDIS VALUE WAS NOT A DICT'
            ...
            for stock_symbol, stock_pipes in latest_model_pipelines.items():
                ...
                for pipe_name, model_sequence in stock_pipes.items():
                    ...
                    misc.log(f"[COMPONENT] DEPLOYING PIPE '{pipe_name}' FOR STOCK '{stock_symbol}'")

                    # CONSTRUCT REQUESTED MODEL
                    for block in model_sequence:
                        assert 'model_name' in block, f"[COMPONENT] MODEL PROPERTY 'model_name' MISSING"
                        assert 'model_version' in block, f"[COMPONENT] MODEL PROPERTY 'model_version' MISSING"
                        ...
                        assert model_name in latest_model_versions, f"[COMPONENT] MODEL '{model_name}' DOES NOT EXIST"

                        # AUDIT -- IF A STRINGIFIED VERSION ALIAS WAS PROVIDED
                        if type(version_number) == str:
                            assert version_number in latest_model_versions[model_name], f"[COMPONENT] MODEL ALIAS '{version_number}' DOES NOT EXIST FOR MODEL '{model_name}'"
                        ...
                        # assert self.mlflow.model_exists(model_name, version_number), f"[COMPONENT] MODEL '{model_name}' VERSION {version_number} DOES NOT EXISTS"

                        # LOAD IN FAKE MODELS -- FOR TESTING
                        model = self.mlflow.load_fake_model(model_name, version_alias, version_number)
                        models.append(model)
                        misc.log(f'[COMPONENT] DEPLOYED MODEL {model_name, version_alias, version_number}')
            ...
            misc.log('[COMPONENT] UPDATED PIPELINE STATE (REDIS)')

        except AssertionError as error:
            misc.log(error)
            misc.log('[COMPONENT] REVERTED PIPELINE STATE UPDATE (REDIS)')
